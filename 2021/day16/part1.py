...
def hex2bin(s:str):
    ...
    for r in conversion_table:
        s = s.replace(r,conversion_table[r])
    return s
...
class packet:
    def __init__(self,data_bin:str):
        self.data_bin = StringIO(data_bin)
        self.read_packet()
        self.subpacket = []

    def read_packet(self):
        self.bits_read = 0
        self.version = int(self.data_bin.read(3),2)
        self.bits_read += 3
        self.id = int(self.data_bin.read(3),2)
        self.bits_read += 3

        if self.id == 4:
            self.value,foo = self.get_literal_value(self.data_bin)
            self.bits_read += foo
        else:
            self.length_type_id = int(self.data_bin.read(1),2)
            self.bits_read += 1
            if self.length_type_id == 0:
                self.total_length_in_bits = int(self.data_bin.read(15),2)
                self.bits_read += 15
            else:
                self.number_of_sub_packets = int(self.data_bin.read(11),2)
                self.bits_read += 11
    
    def get_literal_value(self,d:StringIO):
        count_bits_read = 0
        value = ""
        while True:
            foo = self.data_bin.read(5)
            count_bits_read += 5
            value += foo[1:]
            if foo[0] == '0':
                value = int(value,2)
                break
        return value, count_bits_read
